...
def on_message(client, userdata, msg):
    try:
        ...
        print("data_received",msg.payload.decode())
        data_received = str(msg.payload.decode())
        print(type(data_received))
        if isinstance(data_received,int):
            if data_received > 10 and data_received <= 20:
                send_notification("Inserting the data in the MYSQL table for Edge Analtics")
            if data_received < 0:
        ...
        print("Exception is on message block",e)
...
try:
    ...
    client.connect("192.168.77.53", 1883, 60)
