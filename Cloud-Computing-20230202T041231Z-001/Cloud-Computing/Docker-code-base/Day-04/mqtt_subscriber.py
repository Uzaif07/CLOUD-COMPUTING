...
from notification_handler import send_notifcation
    ...
    try:
        ...
        if isinstance(data_received,int):
            if data_received > 10 and data_received <= 20:
                send_notifcation("Inserting The data in the MYSQL table for Edge Analytics")
            if data_received < 0:
...
try:
    ...
    client.connect("192.168.77.206", 1883, 60)
